<form method="POST" enctype="multipart/form-data">
    ...
    @php $selected = $item->kategori_id ?? ''; @endphp
    <div class="form-group">
        <label>Kategori</label>
        <select class="form-control" required name="kategori_id">
            <option @if ($selected == '') selected @endif value="">--Pilih--</option>
            @foreach ($kategori_items as $kategori)
                <option @if ($selected == $kategori->id) selected @endif value="{{ $kategori->id }}">{{ $kategori->nama }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="form-group">
        <label>Foto</label>
        <input type="file" class="form-control" name="foto" {{ $method === 'edit' ? '' : 'required' }} accept="image/*">
        @if ($method == 'edit')
